...
//		testMLP("data/runMLP");
	...
	public static void testAPEX(String outputDir){
		...
		APEX apex = new APEX(BLOCK_HEIGHT*BLOCK_WIDTH, COMPONENTS, ALPHA);
		double[] errs = apex.train(dataSet, EPOCH_NUM);
		// Analytic solution
	...
	public static void testMLP(String outputDir){
		...
		for(int p = 1; p <= COMPONENTS; p++){
			// Training
			MLP mlp = new MLP(BLOCK_HEIGHT*BLOCK_WIDTH, p, ALPHA);
			double[] errs = mlp.train(dataSet, EPOCH_NUM);
			// Images reconstruction
			for(String image : TEST_IMAGES){
